...
@keti.handle()
async def _(bot: Bot, event: Event, state: T_State):
    anstring = '本周TZ战队课题（11月2日更新）：\n'
    ketiqu = total_list.by_id('11234')
    anstring += song_txt2(ketiqu)
    anstring += f"\n（紫谱）定数: {ketiqu['ds'][3]}\n"
    ketiqu2 = total_list.by_id('11235')
    anstring += song_txt2(ketiqu2)
    anstring += f"\n（红谱）定数: {ketiqu2['ds'][2]}"
    achistr = """本期课题曲目:
    1.last samurai（最后的武士）
        分类:舞萌
        等级:13+
        版本:舞萌2022/maimai splash plus
        あいうえお：ら行
    2. VIIIbit explorer（8bit浏览器）
        分类:舞萌
        等级:13（红谱expert难度）
        版本:舞萌2022/maimai splash plus
        あいうえお：V-Z
    桃子极：合计达成率201%
    桃子极+：合计达成率201.4%
    桃子将：在极的基础上，非p总和不超过12且不允许slide非p
    桃子将+：极+基础上 tap非p总和不超过6且不允许其他非p（break、hold、touch、slide需全p）
    桃子神：将+基础上 dx分数大于等于3650
    课题不做强制要求。完成课题后，请将课题上传到相册:桃子战队第四期课题。没有完成，但刷了大于6pc的，也可以将打的最好的成绩图上传。在ID中挂有tz，课题奖励额外加一分。
    课题说明:新歌课题！日本人可以先打，大陆人可以等周四再打。这次是慢星星和地力课题。"""
    await keti.send(Message([{
        "type": "image",
        "data": {
            "file": f"base64://{str(image_to_base64(text_to_image(achistr)), encoding='utf-8')}"
        }
    }]))
...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    if(len(req) == 2):
        ...
        for t in tmp:
            ...
            tmp2 = tmp2 + t
        f.close()
        with open('src/static/aliases(1).tsv','w',encoding='utf-8') as q:
            q.writelines(tmp2)
            q.close()
...
bpms = on_command('查BPM',aliases = {'!BPM','!bpm','查bpm'})
@bpms.handle()
async def _(bot: Bot, event: Event, state: T_State):
    req = str(event.get_message()).strip().split(" ")
    try:
        ...
        await bpms.finish("咱至少输入两个数字吧QAQ\n请不要在酒吧点炒饭喵")
    if des1 > des2:
        await bpms.finish("BPM范围不能是空集！\n请不要在酒吧点炒饭喵")
    ans = '查询到以下结果: \n\n\n\n\n\n\n\n'
    count = 0
    for musics in total_list:
        if musics['basic_info']['bpm'] >= des1 and musics['basic_info']['bpm'] <= des2:
            ans = ans + f"(BPM {musics['basic_info']['bpm']}) {musics['id']}. {musics['title']} \n\n\n\n\n\n\n"
            count = count + 1
            if count >= 100:
                await bpms.finish("数据超过100条，请不要在酒吧点炒饭喵")
            pass
    ans = ans  + f'\n\n\n共 {count} 条, Generated by Eurobot\n\n\n\n\n\n'
    await bpms.finish(Message([{
            "type": "image",
            "data": {
                "file": f"base64://{str(image_to_base64(text_to_image(ans)), encoding='utf-8')}"
                }
